...
namespace MixedRealityExtension.ProceduralToolkit
{
	...
	public partial class MeshDraft
	{
		...
		public List<Plane> tangents = new List<Plane>();
		...
		/// </summary>
		public MeshDraft(ArrayMesh mesh)
		{
		...
		public MeshDraft AddTriangle(Vector3 vertex0, Vector3 vertex1, Vector3 vertex2, bool calculateNormal)
		{
			if (calculateNormal)
			{
				Vector3 normal = (vertex1 - vertex0).Cross(vertex2 - vertex0).Normalized();
		...
		public MeshDraft AddQuad(Vector3 origin, Vector3 width, Vector3 height, bool calculateNormal)
		{
			...
			if (calculateNormal)
			{
				Vector3 normal = height.Cross(width).Normalized();
		...
		public MeshDraft AddQuad(Vector3 vertex0, Vector3 vertex1, Vector3 vertex2, Vector3 vertex3, bool calculateNormal)
		{
			if (calculateNormal)
			{
				Vector3 normal = (vertex1 - vertex0).Cross(vertex3 - vertex0).Normalized();
				return AddQuad(vertex0, vertex1, vertex2, vertex3, normal, normal, normal, normal);
			}
			return _AddQuad(vertex0, vertex1, vertex2, vertex3);
		}
		...
		{
			Vector3 normal = (fan[1] - fan[0]).Cross(fan[fan.Count - 1] - fan[0]).Normalized();
		...
		{
			Vector3 normal = (strip[1] - strip[0]).Cross(strip[2] - strip[0]).Normalized();
