	...
	public class StraightSkeletonGenerator
	{
		...
		public StraightSkeleton Generate(IList<Vector2> polygon)
		{
			...
			while (activePlans.Count > 0)
			{
				if (count > polygon.Count)
				{
					...
					API.MREAPI.Logger.LogError("Too many iterations");
		...
		private bool OffsetAndDetectEvents()
		{
			...
			foreach (var plan in activePlans)
			{
				...
				{
					// BEGIN: Modified by Microsoft Corporation for generic logging purposes.
					API.MREAPI.Logger.LogError("Unable to find offset");
					// END: Modified by Microsoft Corporation for generic logging purposes.
		...
		private void CleanupLoops()
		{
			foreach (var plan in activePlans)
			{
				...
				{
					// BEGIN: Modified by Microsoft Corporation for generic logging purposes.
					API.MREAPI.Logger.LogError("Invalid plan: " + plan.First);
					// END: Modified by Microsoft Corporation for generic logging purposes.
				}
		...
		private static bool BisectorBisector(Plan.Vertex vertexA, Plan.Vertex vertexB, out float offset)
		{
			...
			if (Intersect.RayRay(vertexA.position, vertexA.bisector, vertexB.position, vertexB.bisector, out intersection))
			{
				...
				{
					float toIntersection = vertexA.position.DistanceTo(vertexB.position);
		...
		{
			float toIntersection = vertex.position.DistanceTo(intersection);
		...
		{
			return (a - b).LengthSquared() < Geometry.Epsilon;
		}
