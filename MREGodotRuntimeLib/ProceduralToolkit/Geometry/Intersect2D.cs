...
namespace MixedRealityExtension.ProceduralToolkit
{
	...
	public static partial class Intersect
	{
		...
		public static bool PointRay(Vector2 point, Vector2 rayOrigin, Vector2 rayDirection)
		{
			...
			return -Geometry.Epsilon < perpDot && perpDot < Geometry.Epsilon &&
				   rayDirection.Dot(toPoint) > -Geometry.Epsilon;
		}
		...
		public static bool PointRay(Vector2 point, Vector2 rayOrigin, Vector2 rayDirection, out int side)
		{
			...
			side = 0;
			return rayDirection.Dot(toPoint) > -Geometry.Epsilon;
		}
		...
		public static bool PointSegment(Vector2 point, Vector2 segmentA, Vector2 segmentB)
		{
			...
			float sqrSegmentLength = fromAToB.LengthSquared();
			...
			{
				float pointProjection = segmentDirection.Dot(toPoint);
		...
		public static bool PointCircle(Vector2 point, Vector2 circleCenter, float circleRadius)
		{
			...
			return (point - circleCenter).Length() < circleRadius + Geometry.Epsilon;
		}
			...
			out IntersectionLineSegment2 intersection)
		{
			...
			if (Mathf.Abs(denominator) < Geometry.Epsilon)
			{
				...
				// Collinear
				bool segmentIsAPoint = segmentDirection.LengthSquared() < Geometry.Epsilon;
				if (segmentIsAPoint)
				{
			...
			out IntersectionRaySegment2 intersection)
		{
			...
			if (Mathf.Abs(denominator) < Geometry.Epsilon)
			{
				...
				bool segmentIsAPoint = segmentDirection.LengthSquared() < Geometry.Epsilon;
				float segmentAProjection = rayDirection.Dot(segmentA - rayOrigin);
				if (segmentIsAPoint)
				{
				...
				float segmentBProjection = rayDirection.Dot(segmentB - rayOrigin);
				if (segmentAProjection > -Geometry.Epsilon)
				{
			...
			out IntersectionSegmentSegment2 intersection)
		{
			...
			float sqrSegment1Length = direction1.LengthSquared();
			float sqrSegment2Length = direction2.LengthSquared();
			...
			out IntersectionSegmentSegment2 intersection)
		{
			...
			float rightAProjection = leftDirection.Dot(rightA - leftB);
			...
			if (rightAProjection < 0)
			{
				...
				//     RA------RB
				Vector2 pointB;
				float rightBProjection = leftDirection.Dot(rightB - leftA);
