...
namespace MixedRealityExtension.ProceduralToolkit
{
	...
	public static partial class Intersect
	{
		...
		public static bool PointSphere(Vector3 point, Vector3 sphereCenter, float sphereRadius)
		{
			...
			return (point - sphereCenter).Length() < sphereRadius + Geometry.Epsilon;
		}
		...
		{
			float sqrMagnitudeA = directionA.LengthSquared();
			float sqrMagnitudeB = directionB.LengthSquared();
			float dotAB = directionA.Dot(directionB);

			float denominator = sqrMagnitudeA*sqrMagnitudeB - dotAB*dotAB;
			Vector3 originBToA = originA - originB;
			float a = directionA.Dot(originBToA);
			float b = directionB.Dot(originBToA);
			...
			if ((closestPointB - closestPointA).LengthSquared() < Geometry.Epsilon)
			{
			...
			out IntersectionSphereSphere intersection)
		{
			...
			float distanceFromBtoASqr = fromBtoA.LengthSquared();
