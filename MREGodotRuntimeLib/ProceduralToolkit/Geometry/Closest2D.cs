...
namespace MixedRealityExtension.ProceduralToolkit
{
	...
	public static partial class Closest
	{
		...
		{
			// In theory, LengthSquared() should be 1, but in practice this division helps with numerical stability
			projectedX = lineDirection.Dot(point - lineOrigin)/lineDirection.LengthSquared();
		...
		{
			float pointProjection = rayDirection.Dot(point - rayOrigin);
			...
			// In theory, LengthSquared() should be 1, but in practice this division helps with numerical stability
			projectedX = pointProjection/rayDirection.LengthSquared();
			...
			out Vector2 pointA, out Vector2 pointB)
		{
			...
			if (Mathf.Abs(denominator) < Geometry.Epsilon)
			{
					...
					Mathf.Abs(VectorE.PerpDot(directionA, originBToA)) > Geometry.Epsilon)
				{
					// Not collinear
					pointA = originA;
					pointB = originB + directionB*directionB.Dot(originBToA);
					return;
				}
			...
			out Vector2 linePoint, out Vector2 segmentPoint)
		{
			...
			if (Mathf.Abs(denominator) < Geometry.Epsilon)
			{
				...
				if (Mathf.Abs(perpDotA) > Geometry.Epsilon || Mathf.Abs(perpDotB) > Geometry.Epsilon)
				{
					...
					{
						float segmentBProjection = lineDirection.Dot(lineOrigin - segmentB);
			...
			{
				// No intersection
				segmentPoint = segmentA + segmentDirection*Mathf.Clamp(segmentDistance, 0, 1);
			...
			out Vector2 rayPoint, out Vector2 segmentPoint)
		{
			...
			if (Mathf.Abs(denominator) < Geometry.Epsilon)
			{
				...
				float segmentBProjection = rayDirection.Dot(rayOriginToSegmentB);
				if (Mathf.Abs(perpDotA) > Geometry.Epsilon || Mathf.Abs(perpDotB) > Geometry.Epsilon)
				{
					// Not collinear
					...
					if (segmentAProjection > -Geometry.Epsilon || segmentBProjection > -Geometry.Epsilon)
					{
						...
						if (sqrSegmentLength > Geometry.Epsilon)
						{
							float rayOriginProjection = segmentDirection.Dot(segmentAToOrigin)/sqrSegmentLength;
				...
				segmentDistance < -Geometry.Epsilon || segmentDistance > 1 + Geometry.Epsilon)
			{
				...
				else if (!segmentAOnRay && segmentBOnRay)
				{
					...
					else
					{
						...
						segmentPoint = segmentA + segmentDirection*originProjection/segmentDirection.LengthSquared();
						return;
					}
				}
				else
			...
			out Vector2 segmentPoint, out Vector2 circlePoint)
		{
			...
			{
				segmentPoint = segmentA;
				float distanceToPoint = segmentAToCenter.Length();
			...
			float sqrDistanceToLine = segmentAToCenter.LengthSquared() - centerProjection*centerProjection;
			float sqrDistanceToIntersection = circleRadius*circleRadius - sqrDistanceToLine;
			if (sqrDistanceToIntersection < -Geometry.Epsilon)
			{
				// No intersection
